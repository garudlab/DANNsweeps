...
class CustomDataGen(Sequence):
   ...
   def __init__(self,gene_sim_neu,gene_sim_hs,gene_sim_ss,gene_sim_neu_tar,gene_sim_hs_tar,gene_sim_ss_tar,batch_size,neutral_train_idx,HS_train_idx,SS_train_idx, **kwargs):#,src_neutral_train_idx,src_sweep_train_idx): 
      '''
      ...
      self.src_neutral =gene_sim_neu[:neutral_train_idx,:,:,:] #[:192000,:,:,:] ##[:,:99,:,:]
      self.src_HS =gene_sim_hs[:HS_train_idx,:,:,:] #[:96000,:,:,:] ##[:,:99,:,:]
      self.src_SS =gene_sim_ss[:SS_train_idx,:,:,:] #[:96000,:,:,:] # #[:,:99,:,:]
      
      #self.tar_neutral =gene_sim_neu_tar #[:192000,:,:,:] ##[:,:99,:,:]
      #self.tar_HS =gene_sim_hs_tar #[:96000,:,:,:] ##[:,:99,:,:]
      #self.tar_SS =gene_sim_ss_tar #[:96000,:,:,:] # #[:,:99,:,:]

      self.tar_data =np.concatenate((gene_sim_neu_tar[:neutral_train_idx,:,:,:],\
                    gene_sim_hs_tar[:HS_train_idx,:,:,:],gene_sim_ss_tar[:SS_train_idx,:,:,:]), axis=0)
...
def create_model(mmap_sim,loss_weights):
    ...
    optimizer =  Adam(learning_rate=1e-5) #, clipnorm=1.0) #0.0005, 0.001 clipvalue=1.0, learning_rate=1e-5
...
def train_model(model,gene_sim_neu,gene_sim_hs,gene_sim_ss,gene_sim_neu_tar,gene_sim_hs_tar,gene_sim_ss_tar,val_split,batch_size,loss_weights,path):
    """
    train CNN model 
    ...
    #sims for validation set (use target domain for validation?)
    neutral_idx =gene_sim_neu_tar.shape[0] #200000#
    HS_idx =gene_sim_hs_tar.shape[0] #100000
    SS_idx =gene_sim_ss_tar.shape[0] #100000
    ...
    val_X_neutral=gene_sim_neu_tar[-neutral_val_idx:,:,:,:] #bottleneck neutral
    val_X_HS=gene_sim_hs_tar[-HS_val_idx:,:,:,:]
    val_X_SS=gene_sim_ss_tar[-SS_val_idx:,:,:,:]
    ...
    val_Y_discr = -1*np.ones(neutral_val_idx+HS_val_idx+SS_val_idx) #-1

    #no validation data
    data_gen=CustomDataGen(gene_sim_neu=gene_sim_neu,gene_sim_hs=gene_sim_hs,gene_sim_ss=gene_sim_ss,\
                gene_sim_neu_tar=gene_sim_neu_tar,gene_sim_hs_tar=gene_sim_hs_tar,gene_sim_ss_tar=gene_sim_ss_tar,\
                batch_size=batch_size,neutral_train_idx=neutral_train_idx,HS_train_idx=HS_train_idx,SS_train_idx=SS_train_idx)
    ...
    accuracy_class_val = score.history['val_classifier_custom_categorical_accuracy']
    accuracy_discr_val = score.history['val_discriminator_custom_binary_accuracy']
    loss_class_val = score.history['val_classifier_loss']
    loss_discr_val = score.history['val_discriminator_loss']


    # Save to a text file
    with open('training_multiclass_results.txt', 'w') as f:
        f.write('Epoch,Loss,class_accuracy,discr_accuracy,class_loss,discr_loss,val_class_accuracy, val_discr_accuracy,val_class_loss,val_discr_loss\n')
        for epoch in range(len(loss)):
            f.write(f"{epoch + 1},{loss[epoch]},{accuracy_class[epoch]},{accuracy_discr[epoch]},{loss_class[epoch]},{loss_discr[epoch]},\
                {accuracy_class_val[epoch]},{accuracy_discr_val[epoch]},{loss_class_val[epoch]},{loss_discr_val[epoch]}\n")
...
def load_grl_model_lambdaScheduler(path):
    optimizer =  Adam(learning_rate=1e-5) #0.0001 learning_rate=1e-5
