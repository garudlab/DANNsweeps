...
H=0.5
chr_len=450000
rho = np.random.uniform(3e-9,2e-8) #draw_truncated_exponential(1e-8, 3e-8)
mu = np.random.uniform(1e-8,1.5e-8)
s=np.random.uniform(0.05,0.1)
sweep_gbp = round(np.random.uniform(840,1500)) #sweep start time # generations ago
PF=1.0 #np.random.uniform(0.5,0.95)
SweepStart=np.random.uniform(0.08,0.25)
adaptive_theta=1.0   #np.random.uniform(1.0,5.0)
...
command += ' -d THETA_A='+str(adaptive_theta)+' Souilmi_Eurasia_postBurnIn_SS.slim'
print(command)
